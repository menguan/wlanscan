...
BOOL CwlanscanDlg::OnInitDialog()
{
	...
	this->m_ctlList.InsertColumn(3,"Security",LVCFMT_LEFT,75);
	this->m_ctlList.InsertColumn(4,"Channel",LVCFMT_LEFT,75);
...
// 	pDlg->m_threadID = GetCurrentThreadId() ;
	...
	while( TRUE )
	{
		...
		//get list
		std::vector<PWLAN_BSS_LIST> pBssList = pDlg->m_airctl.scan();
		pDlg->LabelInfo.SetWindowTextA("scanning..");
		if (!pBssList .empty())
		{
		...
		//
			...
			for (int j = 0; j < pBssList.size(); j++)
			{
				PWLAN_BSS_LIST ppWlanBssList = pBssList[j];

				for (int i = 0; i < ppWlanBssList->dwNumberOfItems; i++)
				{
					WLAN_BSS_ENTRY bssEntry = ppWlanBssList->wlanBssEntries[i];
					lvi.iItem = pDlg->m_ctlList.GetItemCount();
					lvi.iSubItem = 0;

					lvi.pszText = (LPTSTR)(LPCTSTR)(bssEntry.dot11Ssid.ucSSID);
					pDlg->m_ctlList.InsertItem(&lvi);

					lvi.iSubItem = 1;

					sprintf((char*)macadress, "%02X-%02X-%02X-%02X-%02X-%02X", (int*)bssEntry.dot11Bssid[0], (int*)bssEntry.dot11Bssid[1],
						(int*)bssEntry.dot11Bssid[2], bssEntry.dot11Bssid[3], (int*)bssEntry.dot11Bssid[4], (int*)bssEntry.dot11Bssid[5]);
					...
					lvi.iSubItem = 2;
					strItem.Format(_T("%i"), bssEntry.lRssi);
					lvi.pszText = (LPTSTR)(LPCTSTR)(strItem);
					pDlg->m_ctlList.SetItem(&lvi);


					char encryption[32];
					switch (bssEntry.usCapabilityInformation & 16) {
					case 16:
						strcpy(encryption, "encrypted");
						break;

					default:
						//itoa(bssEntry.usCapabilityInformation, encryption, 10);
					...
					lvi.iSubItem = 4;
					int freq = bssEntry.ulChCenterFrequency;
					freq -= 2407000;
					freq /= 5000;
					...
					lvi.iSubItem = 5;
					strItem.Format(_T("%i"), bssEntry.usBeaconPeriod);
					lvi.pszText = (LPTSTR)(LPCTSTR)(strItem);
					pDlg->m_ctlList.SetItem(&lvi);
